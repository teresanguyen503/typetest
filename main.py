...
class main:
    ...
    def run(self):
        ...
        while(self._running):
            ...
            # update user input 
            self.draw_text(self._window, self._input_text, 274, 26, white)
                ...
                if event.type == QUIT:
                    ...
                    if self._active and not self._end:
                        if event.key == pygame.K_RETURN:
                            ...
                            print(self._results)
                            self.draw_text(self._window, self._results, 350, 28, self._result)  
                            self._end = True
    ...
    def draw_text(self, screen, message, y_coord ,size, color):
        font = pygame.font.Font(None, size)
        text = font.render(message, 1, color)
        text_box = text.get_rect(center = (self._width / 2, y_coord))
    ...
    def show_results(self, screen):
        if(not self._end):
            ...
            # Accuracy
            count = 0
            for index, correct in enumerate(self._word):
                try:
                    if self._input_text[index] == correct:
                        count += 1
                except:
                    pass
            ...
            self.draw_text(screen, "Reset", self._height - 35, 25, black)
    ...
    def _reset_game(self):
        ...
        # Get random sentence 
        self._word = self._get_sentence()
        if (not self._word): self._reset_game()
        # heading 
        self._window.fill(black)
        self._window.blit(self._background, (0,0))
        message = "Type Fast and Accurately!!!"
        detailed_message = "**Make sure to click on the input box before you start**"
        self.draw_text(self._window, message, 80, 80, self._header)  
        self.draw_text(self._window, detailed_message, 120, 30, self._details)  
        # input box 
        pygame.draw.rect(self._window, aquamarine, text_input_rect, 2)
        # sentence
        self.draw_text(self._window, self._word, 200, 28, self._text)
